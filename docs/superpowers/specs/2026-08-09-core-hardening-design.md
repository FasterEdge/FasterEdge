...
Atom 内部维护候选注册表、成功挂载快照、挂载顺序、状态和 `sync.RWMutex`。所有 map 均为私有字段。公开读取 API 返回单项值或 map 副本，不泄露可写底层 map。
...
注册拒绝 nil 组件、空白名称和同名组件；不再静默覆盖。组件名称在注册时规范化检查，但保留原始大小写作为键，避免未经约定改变插件标识。
...
5. 外部 context 取消或截止时，监督器把取消信号传播给全部 Runner。
6. 监督器等待 Runner 退出；超过关闭等待时间则返回 `ErrShutdownTimeout`。Go 无法强制杀死 goroutine，因此 Runner 必须响应 context，超时错误用于暴露违反契约的实现。
7. 只有 Runner 全部退出后，才逆序卸载实现 `Unmounter` 的组件。清理完整时 Atom 转为 `Stopped`；卸载失败时转为 `Failed` 并返回汇总错误。
8. 若关闭超时，监督器不卸载仍可能被存活 Runner 使用的资源，Atom 转为 `Failed`。这会有意保留资源而不是制造并发关闭或 use-after-close；错误会明确列出未退出的 Runner。
...
每个内置命令提供导出常量。未知命令使用 `ErrUnsupportedCommand`，参数类型或内容错误使用 `ErrInvalidArguments`，调用方可通过 `errors.Is` 判断。组件使用 `%w` 包装底层错误，不再折叠成诸如 `"fetch failed"` 的无上下文字符串。
